...
for rclass in ResClasses:
  ...
  if (rclass == 'MainChain'):
    arange = allres
    #itertools.permutations([allres, allres])
...
def involves_peptide_bond(prefix, atom_idxs):
  if prefix in prefix2pmd_struct:
    pmd_struct = prefix2pmd_struct[prefix]
  else:    
    pmd_struct = ParmEd.load_file(prefix + '.prmtop')
...
for bond in AllBonds:
  ...
  # If this parameter doesn't involve the peptide bond, skip it
  if not(involves_peptide_bond(prefix, (bond.atom1pos, bond.atom2pos))):
    continue
...
for angle in AllAngls:
  ...
  # If this parameter doesn't involve the peptide bond, skip it
  if not(involves_peptide_bond(prefix, (angle.atom1pos, angle.atom2pos, angle.atom3pos))):
    continue
...
for dihedral in AllDihes:
  ...
  # If this parameter doesn't involve the peptide bond, skip it
  if not(involves_peptide_bond(prefix, (dihedral.atom1pos,
                                        dihedral.atom2pos,
                                        dihedral.atom3pos,
                                        dihedral.atom4pos))):
    continue
